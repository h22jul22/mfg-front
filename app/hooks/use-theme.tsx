...
export function useCorrectCssTransition({ disableTransitions = false }: { disableTransitions?: boolean } = {}) {
  return useCallback(
    (callback: () => void) => {
...
export function PreventFlashOnWrongTheme({ ssrTheme, nonce }: PreventFlashOnWrongThemeProps) {
  const [theme] = useTheme();

  // DOMPurify를 사용하여 XSS 공격 방지
  const sanitizedThemeCode =
    typeof window !== 'undefined'
      ? DOMPurify.sanitize(clientThemeCode, {
          ALLOWED_TAGS: [],
          ALLOWED_ATTR: [],
          KEEP_CONTENT: true,
        })
      ...
      */}
      {ssrTheme ? null : (
        <script
          ...
          // is finished loading.
          dangerouslySetInnerHTML={{ __html: sanitizedThemeCode }}
          nonce={nonce}
        />
      )}
    </>
...
export function useTheme() {
  ...
  if (context === undefined) {
    throw new Response('useTheme must be used within a ThemeProvider', { status: 500 });
  }
  return context;
}
